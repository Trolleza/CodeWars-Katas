...
const allNonConsecutive = (arr) =>
  arr.reduce(
    (pre, n, i) => (i && n - 1 !== arr[i - 1] ? [...pre, { i, n }] : pre),
    []
  );
...
function allNonConsecutive(arr) {
  var nonConsecutiveNums = [];

  for (var i = 0; i < arr.length; i++) {
    if (i > 0 && arr[i] - arr[i - 1] !== 1) {
      nonConsecutiveNums.push({ i: i, n: arr[i] });
    }
  }
...
const allNonConsecutive = (a) =>
  a.map((n, i) => ({ i, n })).filter(({ i, n }) => i > 0 && a[i - 1] + 1 != n);
